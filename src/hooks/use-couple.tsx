...
interface CoupleContextValue {
  ...
  updateCouple: (updates: Partial<Pick<Couple, "name" | "start_date" | "logo_url" | "accent_hue" | "iphone_partner_name" | "android_partner_name" | "monthly_capacity" | "date_tier_limits" | "date_weekly_quota" | "date_monthly_budget">>) => Promise<void>;
}
...
export function CoupleProvider({ children }: { children: ReactNode }) {
  ...
  const updateCouple = useCallback(
    async (updates: Partial<Pick<Couple, "name" | "start_date" | "logo_url" | "accent_hue" | "iphone_partner_name" | "android_partner_name" | "monthly_capacity" | "date_tier_limits" | "date_weekly_quota" | "date_monthly_budget">>) => {
      if (!couple) return;
      const { error } = await supabase
        .from("couples")
        .update(updates)
